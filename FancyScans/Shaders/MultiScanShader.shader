Shader "FancyScans/MultiScanShader" {
	Properties{
		//mesh color
	...
	float _ScanDistance[5] = { 0 , 0 , 0 , 0 , 0};
	//scan line width
	float _ScanWidth;
	...
	void surf(Input IN, inout SurfaceOutput o) {
		...
		fixed isFragOnScanLine = 0;
		for( int i = 0; i < 5; i++){
			isFragOnScanLine += max(0, sign(_ScanDistance[i] - dist)) * max(0, sign(dist - (_ScanDistance[i] - _ScanWidth)));
		}
		isFragOnScanLine = clamp(isFragOnScanLine, 0, 1);
